...
writer = SummaryWriter(f'score/backwardMC-{config.critic_lr}-{config.policy_lr}-{config.normalize_advantages}-BS{config.batch_size}-SS{config.n_samples_per_state}-learnStd={config.learn_std}-{seed}-{int(time.time())}')
